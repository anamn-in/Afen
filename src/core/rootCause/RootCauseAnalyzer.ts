...
export class RootCauseAnalyzer {
  ...
  analyze(input: RootCauseAnalysisInput): RootCauseReport {
    ...
    for (const nodeId of backwardNodes) {
      ...
      let rawStackLines: string[] = [];

      if (node && node.data.metadata) {
        const meta = node.data.metadata;
        rawStackLines = this.extractRawStackLines(meta);
      ...
      if (stackFrames.length > 0 || rawStackLines.length > 0) {
        const result = FrameClassifier.classifyStack(stackFrames, rawStackLines);
  ...
  computeRootCauseForStack(
    ...
    for (const [, entry] of nodeByFrameKey) {
      ...
      const recurrence = graph.getFrameCount(`${frame.filename}|${frame.functionName}|${frame.lineNumber}`);
  ...
  private extractRawStackLines(meta: any): string[] {
    const candidates = [
      meta?.stackTraceRaw,
      meta?.rawStackTrace,
      meta?.stack,
      meta?.causeChain?.stackTraceRaw,
      meta?.original?.stackTraceRaw,
      meta?.telemetry?.stackTraceRaw,
    ];
    ...
    if (Array.isArray(meta?.causeChain?.stackFrames)) {
      return meta.causeChain.stackFrames
        .map((frame: any) => frame.raw || frame.rawLine || frame.source || frame.text)
        .filter(Boolean)
        .map(String);
    }
